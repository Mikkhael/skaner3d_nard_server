...
class Camera {
    ...
    createSocket(_address, _port) {
        ...
        socket.handlers.file.server_error = socket.flop.error.bind(socket.flop);
        socket.handlers.file.format_error = socket.flop.error.bind(socket.flop);
        socket.handlers.downloadSnap.response = (_self, _found, _next)=>{
            console.log(`Download snap response: found[${_found}].`);
            if (!_found) socket.handlers.file.server_error(_self);
            _next();
        };
        socket.handlers.downloadSnap.format_error = socket.flop.error.bind(socket.flop);
...
class CameraCenter {
    ...
    requestStreamFrame() {
        this.cameraList.forEach((x)=>{x.requestStreamFrame()});
    }
